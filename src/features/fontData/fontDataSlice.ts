...
const fontDataSlice = createSlice({
    ...
    initialState,
    reducers: {
        updateActiveFontAxes(state, action: PayloadAction<ActiveFontAxes>) {
            const {fontName, axes} = action.payload;
            if (state.activeFont.fontName === fontName) {
